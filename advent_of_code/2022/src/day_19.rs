...
impl AoCData for Data {
    fn try_new(input: String) -> Option<Self> {
        ...
        for line in input.lines() {
            ...
            // ore bots cost ore
            let ore_bot_costs = [iter.nth(6)?.parse().ok()?, 0, 0, 0];
            // clay bots cost ore
            let clay_bot_costs = [iter.nth(5)?.parse().ok()?, 0, 0, 0];
            // obsidian bots cost ore and clay
            let obsidian_bot_costs = [iter.nth(5)?.parse().ok()?, iter.nth(2)?.parse().ok()?, 0, 0];
            // geode bots cost ore and obsidian
            let geode_bot_costs = [iter.nth(5)?.parse().ok()?, 0, iter.nth(2)?.parse().ok()?, 0];
