...
impl Game {
    ...
    fn run_effects(&mut self) {
        ...
        if self.effect_timers.shield > 0 {
            self.player.armor = 7;
            self.effect_timers.shield = self.effect_timers.shield.saturating_sub(1);
        }
        if self.effect_timers.poison > 0 {
            self.boss.hit_points = self.boss.hit_points.saturating_sub(3);
            self.effect_timers.poison = self.effect_timers.poison.saturating_sub(1);
        }
        if self.effect_timers.recharge > 0 {
            ...
            self.effect_timers.recharge = self.effect_timers.recharge.saturating_sub(1);
        }
    }
    ...
    fn cast_spell(&mut self, spell: &Spell) -> u8 {
        ...
        self.player.mana = self.player.mana.saturating_sub(u16::from(mana));
        mana
    }
    ...
    fn min_mana_to_win(self, hard: bool) -> u32 {
        ...
        }) = pq.pop()
        {
            ...
            for spell in game.possible_spells() {
                ...
                let boss_damage = (game.boss.damage.saturating_sub(game.player.armor)).max(1);
                game.player.hit_points = game.player.hit_points.saturating_sub(boss_damage);
...
impl AoCData for Data {
    ...
    fn part_1(&self) -> String {
        Game::new(self.0).min_mana_to_win(false).to_string()
    }

    fn part_2(&self) -> String {
        Game::new(self.0).min_mana_to_win(true).to_string()
    }
}
