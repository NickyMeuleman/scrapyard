...
#[derive(Debug, Clone)]
pub struct Data(HashMap<usize, Vec<Vec<Cell>>>);
        ...
        for direction in DIRECTIONS.iter() {
            ...
            if !picture.contains_key(&new_coords) {
                ...
                if let Some(found_tile) = search_result {
                    ...
                    let found_idx = available
                        .iter()
                        .position(|tile| tile.id == found_tile.id);
...
fn to_tile_vec(data: &HashMap<usize, Vec<Vec<Cell>>>) -> Vec<Tile> {
...
impl AoCData<'_> for Data {
    ...
    fn part_1(&self) -> AoCResult<impl Display> {
        ...
        let result: usize = side_ids.iter().product();
    ...
    fn part_2(&self) -> AoCResult<impl Display> {
        ...
        let mut available: Vec<Tile> = to_tile_vec(&self.0);
        ...
        Ok(total - (amount_of_monsters * sea_monster_size))
    }
}
