...
impl AoCData<'_> for Data {
    ...
    fn part_1(&self) -> AoCResult<impl Display> {
        ...
        self.0
            ...
            .find(|(_k, v)| {
                v.iter()
                    .all(|(k, v)| one_true_sue.get(k) == Some(v))
            })
            .map(|(k, _v)| k)
        ...
        self.0
            ...
            .find(|(_k, v)| {
                v.iter().all(|(k, v)| match k {
                    SueKey::Cats | SueKey::Trees => one_true_sue.get(k).unwrap() < v,
                    SueKey::Pomeranians | SueKey::Goldfish => one_true_sue.get(k).unwrap() > v,
