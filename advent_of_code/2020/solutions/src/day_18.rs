...
fn do_addition(input: String) -> String {
    ...
    let lhs: i128 = input[start_idx..operator_idx.unwrap()]
        .parse()
        .unwrap();
    let rhs: i128 = input[operator_idx.unwrap() + 1..=stop_idx]
        .parse()
        .unwrap();
    let result = lhs + rhs;
...
impl AoCData<'_> for Data {
    ...
    fn part_1(&self) -> AoCResult<impl Display> {
        let result: i128 = self
            .0
            .iter()
            .map(|line| calculate(line.clone(), false))
            .sum();
    ...
    fn part_2(&self) -> AoCResult<impl Display> {
        let result: i128 = self
            .0
            .iter()
            .map(|line| calculate(line.clone(), true))
            .sum();
