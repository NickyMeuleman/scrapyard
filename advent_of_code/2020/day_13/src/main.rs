...
fn main() {
    ...
    let (target, busses): (i128, Vec<Option<i128>>) = parse(&input);
    println!("Part one answer: {}", part_one(target, &busses));
    println!("Part two answer: {}", part_two(&busses));
}

fn parse(input: &str) -> (i128, Vec<Option<i128>>) {
...
fn part_two(busses: &Vec<Option<i128>>) -> i128 {
    ...
    let mut t = 0;
    let mut mode = 1;
    let busses = busses.iter().enumerate().filter(|(_, &item)| item != None);
...
7,13,x,x,59,x,31,19"
        ...
        assert_eq!(part_one(earliest, &busses), 295);
    }
...
7,13,x,x,59,x,31,19"
        ...
        assert_eq!(part_two(&busses), 1068781);
    }
}
