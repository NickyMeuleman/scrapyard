...
impl<T> Stack<T> {
    ...
    // seperate function for push_node and pop_node, logic not inside push and pop:
    //  to use later when walking the List<T> so called finger
    ...
    fn pop_node(&mut self) -> Option<Box<Node<T>>> {
        self.head.take().map(|mut node| {
            ...
            // the returned Node now has a None in the next field, because we .take() it
            // which is irrelevant since we're only interested in the elem field
            node
        })
    }
